...
class VariableEmbedding(val name: MangledName, override val type: TypeEmbedding) :
    ...
    fun toLocalVarUse(
        ...
        info: Info = Info.NoInfo,
        trafos: Trafos = Trafos.NoTrafos,
    ): Exp.LocalVar = Exp.LocalVar(name, type.viperType, pos)

    override fun toViper(source: KtSourceElement?): Exp = toLocalVarUse(source.asPosition)
    ...
    override fun legacyGetValue(ctx: StmtConversionContext<ResultTrackingContext>, source: KtSourceElement?): ExpEmbedding = this

    override fun legacySetValue(value: ExpEmbedding, ctx: StmtConversionContext<ResultTrackingContext>, source: KtSourceElement?) {
        ctx.addStatement(Stmt.LocalVarAssign(toLocalVarUse(), value.withType(type).toViperExp(TODO()), source.asPosition))
    }
