...
class ContractDescriptionConversionVisitor(
    ...
    override fun visitIsInstancePredicate(
        ...
        val subtypeRel = Is(TypeOfCall(argVar), ctx.embedType(isInstancePredicate.type))
    ...
    private fun VariableEmbedding.nullCmp(isNegated: Boolean, source: KtSourceElement? = null): ExpEmbedding =
        if (type is NullableTypeEmbedding) {
            if (isNegated) {
                NeCmp(this, type.nullVal()).withPosition(source)
            } else {
                EqCmp(this, type.nullVal()).withPosition(source)
            }
        } else {
            BooleanLit(isNegated).withPosition(source)
        }
}
